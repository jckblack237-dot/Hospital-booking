...
async function load() {
  data = await api(`/api/clinic/billing?clinicId=${state.clinic.id}`);
  render();
}
      ...
      h('div', {},
        ...
        h('div.dim.sm', { style: { marginTop: '4px' } },
          data.aasandhaMode === 'degraded'
            ...
            : 'Live API mode.')),
      h('div.row', {},
        h('button.btn', { onClick: () => run(() => api('/api/clinic/billing/claims/submit', { method: 'POST', body: { clinicId: state.clinic.id } }), 'Claims submitted') }, 'Submit draft claims'),
        h('button.btn.ghost', { onClick: () => run(() => api('/api/clinic/billing/claims/adjudicate', { method: 'POST', body: { clinicId: state.clinic.id } }), 'Adjudication simulated') }, 'Simulate adjudication'))),
